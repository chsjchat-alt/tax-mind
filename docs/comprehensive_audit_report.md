# 蒙牛全产业链 AI 内生合规决策大脑 — 18 文件全面评估审查报告

> 审查日期: 2026-07-15（2026-09 按《方案 V4》§5.4 一期边界同步收敛审查范围）  
> 审查范围: 前端页面层 6 文件 + 后端 API 层 5 文件 + 后端核心引擎层 7 文件  
...
| 5 | `backend/app/core/tax_preference.py` | 核心引擎 | 税收优惠政策匹配 | 3.2 |
| 6 | `backend/app/core/intervention.py` | 核心引擎 | 合规干预策略（分层） | 3.4 |
| 7 | `backend/app/core/simulation_engine.py` | 核心引擎 | 税务场景模拟引擎 | 2.8 |

### P1 级（API 接口层，数据正确性的入口）

| # | 文件 | 层级 | 核心功能 | 评分 |
|---|------|------|----------|------|
| 8 | `backend/app/api/risk_scan.py` | API | 风险扫描触发与结果返回 | 3.2 |
| 9 | `backend/app/api/compliance.py` | API | 合规状态与干预策略 | 3.2 |
| 10 | `backend/app/api/simulation.py` | API | 税务模拟与案例库 | 2.8 |
| 11 | `backend/app/api/remediation.py` | API | 整改任务管理 | 1.6 |
| 12 | `backend/app/api/enterprises.py` | API | 企业 CRUD 管理 | 2.4 |

### P2 级（用户界面层）

| # | 文件 | 层级 | 核心功能 | 评分 |
|---|------|------|----------|------|
| 13 | `frontend/src/pages/Dashboard.tsx` | 页面 | 数据驾驶舱首页 | 3.8 |
| 14 | `frontend/src/pages/RiskMap.tsx` | 页面 | 老板/财务双视角风险地图 | 3.4 |
| 15 | `frontend/src/pages/Compliance.tsx` | 页面 | 合规导航 + 干预解读 | 3.8 |
| 16 | `frontend/src/pages/Simulator.tsx` | 页面 | 风险模拟器 | 3.6 |
| 17 | `frontend/src/pages/Remediation.tsx` | 页面 | 整改追踪看板 | 4.4 |
| 18 | `frontend/src/pages/Reports.tsx` | 页面 | 报告中心 | 3.4 |
...
> **这是本次审查发现的最严重的系统性问题，影响全部 6 个前端页面。**
...
### 3.1 致命级 (Critical) — 共 4 项

| # | 文件 | 问题 | 行号 | 影响 |
|---|------|------|------|------|
| CR-1 | `remediation.py` | 所有 ID 参数 `str` 无 UUID 校验 | L30,65,96,217 | 安全漏洞 |
| CR-2 | `enterprises.py` | 所有 ID 参数 `str` 无 UUID 校验 | L68,129,157 | 安全漏洞 |
| CR-3 | `remediation.py` | PUT 端点内嵌 125 行风险重评 + lazy import | L124-L205 | SRP 违反/维护性极差 |
| CR-4 | `Reports.tsx` | `RISK_HERO_CONFIG` 仅 low/medium/high 三个 key | L279-303 | 运行时崩溃 |

### 3.2 严重级 (Severe) — 共 13 项
...
| SE-10 | `compliance.py` | 可变默认参数 `risk_data: dict = {}` | L139 |
| SE-11 | `simulation.py` | Response Schema 完全未使用 | 全部端 |
| SE-12 | `Dashboard.tsx` | 四流匹配度颜色仅三级 | L201-203, 240-243 |
| SE-13 | `remediation.py` | 宽泛 `except Exception` 吞所有异常 | L202-L204 |
...
| HI-5 | `four_flow_match.py` | Jaccard=1.0 数据缺失漏洞 | L154-L155 |
| HI-6 | `tax_preference.py` | 阈值使用 float 而非 Decimal | L49-L56 |
| HI-7 | `compliance.py` | `get_intervention` 数据注入风险 | L139 |
| HI-8 | `compliance.py` | `get_intervention` catch `TimeoutError` 应用 `asyncio.TimeoutError` | L186 |
| HI-9 | `remediation.py` | 状态比较 "done" 非枚举值 | L117 |
| HI-10 | `remediation.py` | `handleStatusChange` 非空断言风险 | L174-180 |
| HI-11 | `intervention.py` | 干预话术硬编码 3 条消息无法动态组合 | L130-L200 |
| HI-12 | `Compliance.tsx` | `Promise.all` 应改用 `allSettled` | L50-63 |
| HI-13 | `frontend` | 跨页面 RiskLevel 五级体系全线缺失 | 多文件 |
| HI-14 | 后端 | Decimal vs float 精度分裂（4 引擎 × 2 API） | 多文件 |
| HI-15 | `Dashboard.tsx` | 无 `useMemo` 优化 `assessments.map()` | L366-400 |

### 3.4 中危级 (Medium) — 共 16 项
...
| ME-8 | `risk_engine.py` | 风险权重仅注释声明无配置外置 | L32-L45 |
| ME-9 | `RiskMap.tsx` | `deriveICRadarData` 阈值硬编码 | L27-71 |
| ME-10 | `RiskMap.tsx` | `BUSINESS_MODEL_DESC` fallback 可能不正确 | L117-119 |
| ME-11 | `RiskMap.tsx` | 决策建议仅三段 | L376-380 |
| ME-12 | `Reports.tsx` | `getRecCategory` 模糊匹配可能误判 | L506-514 |
| ME-13 | `remediation.py` | 风险重评 lazy import 模式 | L128-L135 |
| ME-14 | `compliance.py` | `risk_data` 无白名单校验 | L176 |
| ME-15 | `Reports.tsx` | `handleDownload` API 返回值被忽略 | L67-82 |
| ME-16 | `Simulator.tsx` | 整改成本仅三级 | L152-156 |
...
| 问题 | 影响范围 | 爆炸半径 |
|------|---------|---------|
| RiskLevel 五级未实现 | 全部 6 前端页面 | **全站** |
| Decimal vs float 精度分裂 | 后端 4 引擎 + 2 API | **全部计算模块** |
...
| 优先级 | 编号 | 问题 | 文件 | 预计工时 |
|--------|------|------|------|----------|
| 🔴 P0 | CR-1 | UUID 校验缺失 | `remediation.py` | 0.5h |
| 🔴 P0 | CR-2 | UUID 校验缺失 | `enterprises.py` | 0.5h |
| 🔴 P0 | CR-4 | RISK_HERO_CONFIG 运行时崩溃 | `Reports.tsx` | 1h |

### Phase 1: 核心正确性（本周修复）

| 优先级 | 编号 | 问题 | 涉及文件 | 预计工时 |
|--------|------|------|----------|----------|
| 🔴 P1 | HI-13 | RiskLevel 五级全线实现 | 6 前端页面 | 4h |
| 🔴 P1 | CR-3 | 风险重评重构 | `remediation.py` | 3h |
| 🔴 P1 | SE-3,4 | 消除三重代码重复 | 3 文件 | 3h |
| 🔴 P1 | HI-14 | Decimal 体系统一 | 4 引擎 + 2 API | 3h |
| 🔴 P1 | HI-4 | PENALTY_MULTIPLIERS 合并 | 2 引擎 | 1h |
| 🔴 P1 | SE-1 | 企业统计 N+1 优化 | `enterprises.py` | 1h |
| 🔴 P1 | SE-10 | 可变默认参数修复 | `compliance.py` | 0.5h |

### Phase 2: 高危修复（下周修复）

| 优先级 | 编号 | 问题 | 涉及文件 | 预计工时 |
|--------|------|------|----------|----------|
| 🟠 P2 | HI-5 | Jaccard=1.0 漏洞修 | `four_flow_match.py` | 1h |
| 🟠 P2 | HI-8 | Schema 空置补全 | `simulation.py`, `compliance.py` | 2h |
| 🟠 P2 | HI-10 | 非空断言移除 | `remediation.tsx` | 0.5h |
| 🟠 P2 | HI-7 | risk_data 注入防护 | `compliance.py` | 1h |
| 🟠 P2 | HI-9 | "done"→枚举值 | `remediation.py` | 0.5h |
...
| 🟡 P3 | HI-15 | useMemo 优化 | `Dashboard.tsx` | 1h |
| 🟡 P3 | 低危项 | 硬编码外置、阈值常量化 | 多文件 | 3h |
| 🟡 P3 | ME-15 | handleDownload 逻辑澄清 | `Reports.tsx` | 0.5h |
...
| 层级 | 平均分 | 主要问题 |
|------|--------|---------|
| 前端页面层 | 3.73 | RiskLevel 五级未实现 |
| 后端 API 层 | 2.64 | UUID 缺失 + 代码重复 |
| 后端引擎层 | 3.34 | float vs Decimal 精度 |
| **全栈** | **3.28** | **跨层一致性严重不足** |

### 7.3 核心发现

1. **RiskLevel 五级体系是最严重的系统性问题** — 虽已在 types/index.ts 和 RISK_COLORS/RISK_LABELS 中完整定义，但全部 6 个前端页面均未实现 medium_high 和 critical 的处理逻辑，这是从代码审查到上轮修复中均未发现的深层漏洞

2. **后端代码复用严重不足** — 风险重评逻辑在 3 处实现，PENALTY_MULTIPLIERS 在 2 处定义且值不一致

3. **金融计算精度不统一** — Decimal（penalty_calculator）vs float（其余 4 引擎）混合，可能导致舍入误差
...
系统整体架构设计合理，分层清晰。核心问题集中在跨层一致性（RiskLevel 五级、Decimal/float、PENALTY_MULTIPLIERS）和代码重复（风险重评三重冗余）。**建议 Phase 0-1 优先解决 4 个致命和 13 个严重问题**，预计总工时约 18h。修复完成后系统可达到 3.8+ 的综合健康度。
